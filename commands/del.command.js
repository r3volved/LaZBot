...
class DelCommand extends Command{
	...
	constructor(config, message) {
		...
		this.helpText = this.config.settings.help.DEL;
	}
	...
	process() {
    	...
    	if( messageParts.length < 2 ) { 
        	...
        	this.messageHandler.sendMessage(this.config.settings.error.DEL_HELP); 
	...
	doDel( messageParts ) {
    	...
    	this.queryHandler.query( this.config.settings.command.del, del ).then(( response ) => {
