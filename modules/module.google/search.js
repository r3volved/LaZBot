...
async function doSearchMeme( obj ) {
  try {

    //Args passed to command
    let { text } = obj.command.args;
    
    //Do stuff here for doSearchMeme
    //...
    let result, message = null;
    try {
        message = await obj.message.reply('Searching for \''+text+'\'... ');
    ...
    let searchUrl = `https://me.me/s/${encodeURIComponent(text)}%20random`;
    try {
        result = await fetchGoogleImage(searchUrl);
        if( !result ) { return obj.fail('Sorry, I could not find any adequate results'); }
    } catch(e) { obj.error('doSearch.fetchGoogle',e); }
    
    message.edit(`Result found!`,{ file:result });
    return obj.success();
...
async function fetchGoogleImage(searchUrl) {
    ...
    return new Promise((resolve, reject) => {
        ...
        snekfetch.get(searchUrl).then((result) => {
            ...
            // This is allowing us to grab the URL from within the instance of the page (HTML)
            let googleData = $('.grid-item').first().find('img').first();
